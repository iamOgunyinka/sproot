...
def delete_exam_info_route():
    try:
        ...
        if not reference_id:
            return error_response('Invalid reference_id')
...
def delete_repository_route():
    ...
    if repository_name is None:
        return error_response('A repository name must be specified')
    for repo in current_user.repositories:
...
def delete_course_route():
    ...
    if course_name is None:
        return error_response('A course name must be specified')
    repository = db.session.query( Repository ).filter_by( repo_name = repository_name,
                                                           owner_id = current_user.id ).first()
    if repository is None:
        return error_response('Cannot find repository housing the course')
    course = db.session.query( Course ).filter_by( repo_id = repository.id, name = course_name ).first()
    if course is None:
        return error_response('Cannot find the course specified')
